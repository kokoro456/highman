...
const allStatusOptions: { value: AllBookingStatus; label: string; color: string }[] = [
  { value: 'CONFIRMED', label: '확정', color: 'bg-brand-500' },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {statusPopup && (
        <div
          ref={statusPopupRef}
          className="fixed z-30 rounded-2xl bg-white ring-1 ring-zinc-200/50 shadow-soft-xl p-2 min-w-[140px]"
          style={{
            left: statusPopup.x,
            top: statusPopup.y,
        ...
        >
          ...
          {allStatusOptions.map((opt) => (
            <button
              ...
              className="flex items-center gap-2 w-full rounded-lg px-3 py-2 text-sm text-zinc-700 transition-colors duration-150 hover:bg-zinc-50 active:bg-zinc-100 disabled:opacity-50"
            >
